...
// Reconciler reconciles a CustomSecret object
...
//+kubebuilder:rbac:groups=bdc.bdos.io,resources=customsecrets,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=bdc.bdos.io,resources=customsecrets/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=bdc.bdos.io,resources=customsecrets/finalizers,verbs=update
...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Lookup the customSecret instance for this reconcile request
	var customSecret bdcv1alpha1.CustomSecret
	if err := r.Get(ctx, req.NamespacedName, &customSecret); err != nil {
		return ctrl.Result{}, client.IgnoreNotFound(err)
	}
	//klog.InfoS("customSecret", "bdc.cs", customSecret)

	// Set BigDataCluster as metadata.ownerReferences
	var bigDataCluster bdcv1alpha1.BigDataCluster
	if err := r.Get(ctx, client.ObjectKey{Name: customSecret.GetAnnotations()[constants.AnnotationBDCName]}, &bigDataCluster); err != nil {
		return ctrl.Result{}, client.IgnoreNotFound(err)
	}
	...
	err := r.patchOwnerReferencer(ctx, &customSecret)
	if err != nil {
	...
	bdcFile, err := bdcParser.GenerateBigDataClusterFile(ctx, &customSecret)
	if err != nil {
	...
	// klog.InfoS("CustomSecret", "output manifests", manifests)
	...
	// UpdateStatus
	customSecret.Status.SchemaConfigMapRef = bdcFile.BDCTemplate.FullTemplate.XDefinitionSchemaName
	err = r.UpdateStatus(ctx, &customSecret)
	if err != nil {
		err = condition.PatchCondition(ctx, r, &customSecret,
			conditiontype.ReconcileError(fmt.Errorf(constants.ErrCreateBDCResource, customSecret.Kind, customSecret.Name, err)))
		if err != nil {
			return ctrl.Result{}, err
		}
		return ctrl.Result{}, err
	}
	err = condition.PatchCondition(ctx, r, &customSecret, conditiontype.ReconcileSuccess())
	if err != nil {
		return ctrl.Result{}, err
	}
...
func (r *Reconciler) UpdateStatus(ctx context.Context, bdc *bdcv1alpha1.CustomSecret, opts ...client.SubResourceUpdateOption) error {
...
func (r *Reconciler) patchOwnerReferencer(ctx context.Context, bdc *bdcv1alpha1.CustomSecret) error {
