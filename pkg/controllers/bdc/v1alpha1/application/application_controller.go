...
import (
	...
	"bpaas-core-operator/api/bdc/v1alpha1"
	...
	"bpaas-core-operator/pkg/controllers/utils/vela"
...
//+kubebuilder:rbac:groups=bdc.bdos.io,resources=applications,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=bdc.bdos.io,resources=applications/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=bdc.bdos.io,resources=applications/finalizers,verbs=update
...
func (reconciler *ApplicationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := reconciler.Get(ctx, client.ObjectKey{Name: application.GetAnnotations()[constants.AnnotationBDCName]}, &bigDataCluster); err != nil {
		// if BigDataCluster is not existed, just clean the application
		...
		// tell maintainer the bdc is not exist or other error
...
func (reconciler *ApplicationReconciler) deleteFinalizer(ctx context.Context, application bdcv1alpha1.Application) {
...
func (reconciler *ApplicationReconciler) handlerFinalizer(ctx context.Context, application bdcv1alpha1.Application, bigDataCluster bdcv1alpha1.BigDataCluster) (bool, error) {
...
func (reconciler *ApplicationReconciler) patchOwnerReferencer(ctx context.Context, application *bdcv1alpha1.Application) error {
...
func (reconciler *ApplicationReconciler) UpdateStatus(ctx context.Context, application *bdcv1alpha1.Application, opts ...client.SubResourceUpdateOption) error {
...
func (reconciler *ApplicationReconciler) reconcileStatusWithInitializeError(ctx context.Context, application bdcv1alpha1.Application, err error) error {
	if err == nil {
		return nil
	}
...
func (reconciler *ApplicationReconciler) reconcileStatusWithOutputDefError(ctx context.Context, application bdcv1alpha1.Application, err error) error {
	if err == nil {
		return nil
	}
...
// reconcile applicaiton status to sync vela application status.
// Actually this controller just generate vela application not for final workload(like deployment/sts),
// so it will use vela appliaiton status directly
func (reconciler *ApplicationReconciler) reconcileStatus(ctx context.Context, application bdcv1alpha1.Application, xDefinitionSchemaName string, DownStreamNamespace string) error {
	if application.Status.Status == "" {
