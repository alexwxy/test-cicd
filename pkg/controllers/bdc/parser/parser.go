...
import (
	...
	"bpaas-core-operator/pkg/controllers/bdc/defcontext"
	"bpaas-core-operator/pkg/controllers/bdc/deftemplate"
	"bpaas-core-operator/pkg/controllers/utils/uuid"
...
func (p *Parser) GenerateBigDataClusterFile(ctx context.Context, bdcObj runtime.Object) (*BDCFile, error) {
	...
	case *bdcv1alpha1.CustomSecret:
		...
		objUID = string(bdcObject.UID)
		specName = bdcObject.Spec.Name
	case *bdcv1alpha1.CustomSetting:
		objName = bdcObject.Name
		objAnnotations = bdcObject.Annotations
		objLabels = bdcObject.Labels
		objSpec = bdcObject.Spec
		...
		objUID = string(bdcObject.UID)
		specName = bdcObject.Spec.Name
	case *bdcv1alpha1.ResourceControl:
		objName = bdcObject.Name
		objAnnotations = bdcObject.Annotations
		objLabels = bdcObject.Labels
		objSpec = bdcObject.Spec
...
func (p *Parser) GetK8sBdcCtxNamespaced(ctx context.Context) (map[string]string, error) {
	...
	err := p.Client.Get(ctx, client.ObjectKey{Name: defcontext.ContextCongfigMap, Namespace: pkgcommon.SystemDefaultNamespace}, &observedConfigMap)
	if err != nil {
		if apierrors.IsNotFound(err) {
			klog.ErrorS(err, "not found", "ConfigMap", defcontext.ContextCongfigMap)
			return nil, nil
		}
		klog.ErrorS(err, "fail to get ConfigMap", "ConfigMap", defcontext.ContextCongfigMap)
		return nil, err
	}

	return observedConfigMap.Data, nil
}
